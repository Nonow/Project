...
public class FenEchec extends JFrame{
	 
		private Echequier e; // echiquier
		...
		public FenEchec(String titre, int w, int h)
		{
			super(titre);
			this.initialise();
			this.setSize(w,h);
		...
		public void initialise()
		{
			...
			e = new Echequier(); // création de l'échiquier
						...
						if((e.getCase(colonneClic, ligneClic).getPiece() != null | pieceTampon != null) )
						{
							...
							else
							{
								...
								if ((pieceTampon.estValide(deplacement) && e.cheminPossible(deplacement)) | e.captureParUnPionPossible(deplacement))
								{
									...
									{
										if(JOptionPane.showConfirmDialog(null, "Flicitation vous avez gagn ! Dsirez-vous jouer de nouveau ?\n", "Mine !", JOptionPane.YES_NO_OPTION) == 0){
											RAZ();
...
*/
		...
		public static void main(String[] args) {
			FenEchec j = new FenEchec("Echec",100,130);
			j.setVisible(true);
